...
test('11. политика загрузки: очередь по расстоянию вместо порогов видимости', () => {
  // Прежний контракт требовал «широкое упреждение, узкая выгрузка» и был
  // опровергнут замером на production: при пороге выгрузки в шесть экранов за
  // один проход по меню src снимался у 117 уже загруженных фотографий (при 82
  // уникальных), и человек ждал повторного декода. Порогов больше нет — есть
  // очередь, упорядоченная по расстоянию, и выгрузка только под бюджет памяти.
  assert.match(APP, /const IMG_MAX_INFLIGHT=\d+;/, 'глубина очереди задана явно');
  assert.match(APP, /const IMG_NEAR_AHEAD=\d+;/, 'упреждение задано явно');
  assert.match(APP, /const IMG_LOADED_BUDGET=\d+;/, 'потолок удерживаемых фотографий задан явно');
  const inflight = Number(APP.match(/const IMG_MAX_INFLIGHT=(\d+);/)[1]);
  const budget = Number(APP.match(/const IMG_LOADED_BUDGET=(\d+);/)[1]);
  assert.ok(inflight >= 3 && inflight <= 6,
    'замер: ниже трёх очередь не успевает заполнять, выше шести канал дробится и ближайшая фотография приезжает позже');
  assert.ok(budget > 120,
    'бюджет — защита от патологически длинного меню, а не рабочий режим: на реальном меню (82 фотографии) он не должен срабатывать');
  assert.ok(!/IMG_EVICT_BEYOND/.test(APP),
    'выгрузка по расстоянию удалена — именно она заставляла готовое фото грузиться заново');
  assert.match(APP, /fetchpriority/, 'приоритет для реально видимых карточек');
  assert.match(APP, /function pumpPhotoQueue/, 'загрузкой управляет очередь, а не наблюдатель');
});
