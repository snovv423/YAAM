...
test('published HTML cache-busts both changed runtime scripts with one cutover version', () => {
  ...
  assert.match(html, /href="css\/style\.css\?v=dish-safe-area-1"/);
  assert.match(html, /src="js\/api\.js\?v=dish-safe-area-1"/);
  assert.match(html, /src="js\/app\.js\?v=dish-safe-area-1"/);
});
