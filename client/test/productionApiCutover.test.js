...
test('published HTML cache-busts both changed runtime scripts with one cutover version', () => {
  ...
  assert.match(html, /href="css\/style\.css\?v=dish-card-frame-1"/);
  assert.match(html, /src="js\/api\.js\?v=dish-card-frame-1"/);
  assert.match(html, /src="js\/app\.js\?v=dish-card-frame-1"/);
});
