...
//
// `card2x` — тот же кадр, что и `card`, только с достаточным числом пикселей
// для HiDPI. Измерено на production: карточка меню занимает 362 CSS px на
// iPhone при DPR 3 (нужно 1086 px) и 541 CSS px на Retina-десктопе при DPR 2
// (нужно 1082 px), а отдавался card шириной 800 — то есть браузер растягивал
// растр в 1.35-1.36 раза. 1200 px закрывает оба случая с небольшим запасом и
// при этом заметно легче full (1920). Качество намеренно равно card (92), а не
// выше и не ниже: у обоих вариантов должна быть одна и та же обработка, иначе
// при переключении между ними на разных устройствах поехали бы цвет и
// микроконтраст. Ни один существующий вариант не тронут.
const VARIANTS = {
  thumb: { maxEdge: 320, quality: 88, public: true },
  card: { maxEdge: 800, quality: 92, public: true },
  card2x: { maxEdge: 1200, quality: 92, public: true },
