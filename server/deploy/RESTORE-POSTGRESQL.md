...
Актуально на 2026-08-03 (Stage 20).
...
| Артефакт | Путь / ключ |
|---|---|
| Локальные копии (все базы) | `/var/backups/yaam/<база>_<UTC>.dump` |
...
- не переносит схему между базами: у `yaam_production` цепочка старая, у
  `yaam_staging_v2` и `yaam_hqtest` — актуальная.
...
| Better Stack heartbeat | отправляется **только** после успешной выгрузки **всех** баз |

Сбой любой из баз даёт ненулевой код возврата unit'а и `/fail` на heartbeat.
Частичный успех успехом не считается.

## Судьба `yaam_production`

Старая база **намеренно оставлена нетронутой** — не удалена и не
переименована. Она продолжает копироваться локально и offsite и остаётся
точкой отката до отдельного решения владельца. Пока она существует, откат
api-pg на состояние до Stage 20 возможен в любой момент.
