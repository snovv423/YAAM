...
echo "(server/deploy/nginx-yaam-postgresql.conf уже размещён в"
...
echo "== Выпуск сертификата (certbot --nginx, автоматически правит конфиг Nginx) =="
sudo certbot --nginx \
  -d "${DOMAIN}" \
  --email "${EMAIL}" \
  --agree-tos \
...
echo "1. curl -I https://${DOMAIN}/health/live — подтвердить HTTPS отвечает"
echo "2. Проверить HSTS-заголовок в ответе (Strict-Transport-Security)"
echo "3. Проверить редирект HTTP -> HTTPS: curl -I http://${DOMAIN}/health/live"
echo "4. См. postgresql-deployment-runbook.md, раздел 'SSL validation'"
