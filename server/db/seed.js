...
const insertRestaurant = db.prepare(`
  INSERT INTO restaurants (name, cuisine, photo_url, cities, hours, delivery_price, min_order, is_open, is_new, rating, rating_count)
  VALUES (:name, :cuisine, :photo_url, :cities, :hours, :delivery_price, :min_order, :is_open, :is_new, :rating, :rating_count)
`);
...
function seedRestaurant(data) {
  const info = insertRestaurant.run({
    ...
    cities: JSON.stringify(data.cities), hours: data.hours,
    delivery_price: data.deliv, min_order: data.min,
...
seedRestaurant({
  ...
  hours: '10:00–23:00', deliv: 150, min: 600, open: true, isNew: false, rate: 4.8, votes: 215,
  menu: [
...
seedRestaurant({
  ...
  hours: '08:00–23:00', deliv: 120, min: 400, open: true, isNew: true, rate: 4.7, votes: 73,
  menu: [
    { cat: 'Кофе', items: [
