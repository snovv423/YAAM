-- =========================================================================
-- 0001_baseline — ЗАМОРОЖЕННЫЙ СНИМОК СХЕМЫ YAAM
-- =========================================================================
--
-- ЭТОТ ФАЙЛ НЕИЗМЕНЯЕМ. Он зафиксирован 2026-08-03 и описывает схему в том
-- виде, в каком она существует на текущей staging-базе — то есть ДО
-- добавления hq_sessions (оно вынесено в 0002).
--
-- ПОЧЕМУ САМОСТОЯТЕЛЬНЫЙ СНИМОК, А НЕ ССЫЛКА НА schema.sql.
-- Первая редакция этой миграции была заглушкой `SELECT 1;`, а runner в
-- момент выполнения читал ТЕКУЩИЙ db/postgresql/schema.sql. Это ломало сам
-- смысл миграции: её содержание менялось вместе с проектом, две базы,
-- «применившие 0001» в разное время, получали РАЗНЫЕ схемы, а контрольная
-- сумма защищала пустую заглушку вместо реально выполненного SQL.
--
-- Теперь baseline самодостаточен: что бы дальше ни происходило со
-- schema.sql, применение 0001 на пустой базе всегда даёт ровно это
-- состояние. Все последующие изменения — только отдельными файлами 0002+.
--
-- НЕ РЕДАКТИРОВАТЬ. Изменение этого файла ломает контрольную сумму на всех
-- базах, где он уже применён, и запуск будет остановлен.
...
-- BEGIN/COMMIT снимка удалены: транзакцию открывает и закрывает runner
-- (services/postgresql/migrator.js). Вложенный COMMIT здесь закрыл бы
-- внешнюю транзакцию, и отметка в schema_migrations перестала бы быть
-- атомарной вместе со схемой.
