...
// YAAM HQ Stage 6 — единственные DATE-колонки (без времени/таймзоны) во всей
// схеме — restaurant_contracts.signed_at/starts_at/ends_at (см.
// db/postgresql/schema.sql). `pg` по умолчанию парсит DATE (OID 1082) в JS
// Date на ПОЛУНОЧЬ ЛОКАЛЬНОГО часового пояса процесса — известная ловушка
// node-postgres: при положительном смещении от UTC (например, Europe/Moscow,
// UTC+3) эта полночь при выводе через любой UTC-based форматтер (например,
// getUTCDate()) оказывается предыдущим календарным днём. Дата "2026-01-15",
// один раз сохранённая, начинала бы визуально "съезжать" на 2026-01-14 при
// каждом повторном показе формы редактирования договора. Строковый парсер
// (значение остаётся 'YYYY-MM-DD', как его вернул сам Postgres) полностью
// убирает эту потенциальную двусмысленность — календарная дата не имеет
// часового пояса, ей не нужен JS Date object. Все остальные даты в схеме —
// TIMESTAMPTZ (настоящие моменты времени с часовым поясом), их парсинг не
// затронут.
