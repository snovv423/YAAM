...
- `server.postgresql.js` не монтирует `routes/postgresql/api.js`/`admin.js`/
  bot — сознательно, вне мандата Stage 6 (см. `postgresql-migration-status.md`,
  раздел Stage 6). Даёт полный, протестированный lifecycle-скелет, в который
  такое монтирование добавляется на Stage 7/8 без переписывания механики
  bootstrap/health/graceful-shutdown.
