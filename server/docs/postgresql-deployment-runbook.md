...
**Stage 5B.2 — фотографии (`MEDIA_LOCAL_ROOT`) в ту же стратегию.** Фотографий
немного (масштаб YAAM — один регион), поэтому не нужна отдельная сложная
система — тот же `pg_dump`, дополненный простым архивом каталога, в то же
расписание:

```bash
# Сразу следом за pg_dump выше — тот же timestamp, тот же принцип "снимок
# согласованного момента". tar не требует остановки backend (фотографии
# пишутся atomic rename — см. services/hq/media/provider.js — снимок в
# худшем случае не увидит самый последний файл, но никогда не увидит
# частично записанный).
sudo tar -czf "/var/backups/yaam/yaam_media_$(date +%Y%m%d_%H%M%S).tar.gz" \
  -C /var/lib/yaam/media .
```

Restore — распаковать в НОВЫЙ путь рядом (не поверх рабочего
`MEDIA_LOCAL_ROOT`), проверить содержимое, только затем подменить и
перезапустить backend — см. `server/docs/persistent-local-media-runbook.md`
за полным restore-drill. Offsite-копия обязательна (тот же принцип, что и
для `pg_dump` выше — не тот же диск VPS). БД-дамп и медиа-архив с одного и
того же расписания достаточно близки по времени для практической
консистентности (owner сам загружает единичные фото не чаще нескольких раз в
день — не финансовые транзакции, требующие point-in-time согласованности
секунда-в-секунду).
