...
- Уже использовавшаяся ранее (Stage 1 и раньше) документация `POST /v3/
  payments`, `GET /v3/payments/{id}`, `POST /v3/refunds`, `GET /v3/refunds/
  {id}` — переиспользована без изменений (`yookassaProvider.js`'s
  `createPayment`/`getStatus`/`refund`/`getRefund` не менялись).
...
3. `payload.type === 'notification'` и `payload.event` — один из
   `payment.succeeded`/`payment.canceled` (единственные два события реально
   обрабатываемого MVP-потока, capture=true СБП, см. ADR) — что-либо ещё
   (включая корректные, но не подписанные в MVP `refund.*`) → `null`,
   **без сетевого вызова** (проверено тестом).
4. `object.id` — непустая строка, иначе `null`.
5. **Каноническая проверка** — `this.getStatus(object.id)` (уже реализован
...
6. Канонический статус ОБЯЗАН совпадать с ожидаемым для заявленного event
   ...
   несовпадение → `null`.
7. Возвращает `{ providerPaymentId, status, amount, currency }` — `amount`/
   `currency` берутся из ТЕЛА уведомления (уже прошедшего каноническую
   проверку id/статуса, но не суммы — provider ничего не знает о нашей БД).
...
`getStatus()` возвращает только нормализованную строку статуса (не сумму) —
сверка суммы структурно не может произойти в provider-слое, который ничего
не знает о нашей БД. Она выполняется в `routes/postgresql/api.js`, ПОСЛЕ
успешной верификации: `event.amount` (число или строка вида `"500.00"`)
сравнивается с сохранённым `payments.amount` (целое число рублей),
...
`routes/postgresql/api.js`: `express.raw({ type: '*/*', limit: '64kb' })` —
...
`server/test/postgresql/paymentSafetyStage8.test.js` — 30 тестов, разделы
A (webhook authenticity, 11 тестов — реальный `YookassaProvider` против
...
- **Реальная сетевая проверка против боевого аккаунта ЮKassa** — вся
  верификация протестирована против управляемого fake-транспорта; реальная
  сеть/боевые credentials — Stage 9/10.
...
- **`refund.succeeded` webhook-подписка** — не добавлена (не требуется для
  MVP: синхронный ответ `refund()` уже даёт подтверждение для СБП) —
  явное, осознанное ограничение scope, не забытый кейс.
