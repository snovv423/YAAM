...
| 18 | Production Switch — Stage 3 (PostgreSQL bot, `server/bot/postgresql/index.js`) | этот же коммит — точный hash самоссылочен и не фиксируется здесь (см. `git log -1` или последний коммит на ветке) |
...
Ничего не осталось из этой категории после Stage 3 — оставшиеся два пункта
(отсутствие ownership-проверки, отсутствие `UNIQUE` на `telegram_chat_id`)
не являются рисками ЭТОЙ миграции — это унаследованные от SQLite-оригинала
продуктовые решения, зафиксированные в `postgresql-bot-port.md` для
будущего явного product/security-решения, не баг переноса.
...
## Последние подтверждённые результаты (после Production Switch Stage 3)

| Набор | Результат |
|---|---|
| PostgreSQL (`npm run test:postgresql`, включая новый `botStage3.test.js`) | 350/352 passed, 2 skipped (явно, с причиной) |
| PostgreSQL `concurrency.test.js` отдельно | 14/14 |
| SQLite (`npm test`, включая существующий HTTP integration suite) | 333/333 |
| **Итого** | **697/699** (2 skipped, 0 failed) |

Полный агрегатный прогон (`test/postgresql/*.test.js`) — минимум два чистых
прогона подряд, без флапанья на повторах (350/352 + 2 skip оба раза). Stage 3
suite (`botStage3.test.js`) — 36/36, запущен 5 раз подряд, стабильно зелёный
на всех прогонах. Отдельно — 20-итерационный стресс-прогон конкурентных
"Принять"-кликов на один заказ: 0 повреждений данных, каждая итерация
получила ровно 2 корректных `answerCallbackQuery` без необработанных
исключений. Ранее (Stage 1/2) наблюдавшийся 1 несвязанный flake в
`orderServiceWave5.test.js` (тайминг-чувствительный SERIALIZABLE-retry сценарий
под нагрузкой параллельных test-файлов, не касается bot-кода) на этот раз не
воспроизвёлся ни на одном из прогонов.
...
(без реального token/сети). Production-приложение по-прежнему полностью на
SQLite — переключение (Stage 8 будущей последовательности) не выполнено и
не запланировано как часть Stage 1/2/3.

## Что осталось до Production Switch

1. **Stage 4 — `routes/admin.js`** — не портирован.
2. **Stage 5 — restaurant pause, оставшаяся часть** — `pauseRestaurant`/
   `resumeRestaurant` перенесены Stage 3 (нужны боту); `sweepPauseExpiry`
   (периодический свип, вызывается из `server.js` `setInterval`) — не
   перенесён, вне scope изолированных wave/stage-модулей.
3. **Stage 6 — операционная обвязка** — скрипт применения схемы к реальной
...
4. **Stage 7 — staging-прогон** и **Stage 8 — сам Production Switch**
   (инфраструктурный, DNS/reverse proxy) — не выполнялись.
5. **YooKassa production validation** — `createPayment()`/`getStatus()`/
...
6. **`YookassaProvider.verifyWebhook()`** — единственный метод провайдера,
...
9. **Продуктовое решение по двум унаследованным от SQLite-оригинала
   ограничениям bot'а** (не блокер Stage 4, но требует отдельного явного
