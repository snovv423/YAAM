...
| 20 | Production Switch — Stage 5 (PostgreSQL restaurant-pause scheduler, `server/services/postgresql/scheduler.js`) | этот же коммит — точный hash самоссылочен и не фиксируется здесь (см. `git log -1` или последний коммит на ветке) |
...
Ничего не осталось из этой категории после Stage 5 — унаследованные от бота
...
переноса. Stage 5 не добавил новых рисков этой категории — все проверенные
race conditions (см. раздел Stage 5 выше) оказались безопасны по конструкции,
без необходимости в SERIALIZABLE/блокировках.
...
## Последние подтверждённые результаты (после Production Switch Stage 5)

| Набор | Результат |
|---|---|
| PostgreSQL (`npm run test:postgresql`, включая новый `schedulerStage5.test.js`) | 415/417 passed, 2 skipped (явно, с причиной) |
| PostgreSQL `concurrency.test.js` отдельно | 14/14 |
| SQLite (`npm test`, включая существующий HTTP integration suite) | 333/333 |
| **Итого** | **762/764** (2 skipped, 0 failed) |

Полный агрегатный прогон (`test/postgresql/*.test.js`) — два чистых прогона
подряд, без флапанья (415/417 + 2 skip оба раза). Stage 5 suite
(`schedulerStage5.test.js`) — 21/21, запущен 5 раз подряд, стабильно зелёный
на всех прогонах (включая тайминг-чувствительные start/stop/concurrent-timer
тесты). Ранее наблюдавшийся несвязанный flake в `orderServiceWave5.test.js`
на прогонах этой задачи не воспроизвёлся.
...
конструкции — ни SERIALIZABLE, ни блокировки не потребовались. Production-приложение
по-прежнему полностью на SQLite — переключение (Stage 8 будущей
последовательности) не выполнено и не запланировано как часть Stage 1-5.

## Что осталось до Production Switch

1. **Stage 6 — операционная обвязка** — скрипт применения схемы к реальной
   ...
   (`await db.close()`), реальное подключение `scheduler.js`/`sweepTimeouts`/
   `sweepStuckRefunds` к работающему процессу (сейчас все изолированы, ни
   один не запускается автоматически). Ничего из этого не создавалось ни в
   одной волне.
2. **Stage 7 — staging-прогон** и **Stage 8 — сам Production Switch**
...
5. **Продуктовое решение по найденному маршрутному багу admin.js**
   ...
   ЖИВОЙ SQLite-admin, не блокер Stage 6+, но требует отдельного явного
...
8. **Продуктовое решение по двум унаследованным от SQLite-оригинала
   ограничениям bot'а** (не блокер Stage 6+, но требует отдельного явного
