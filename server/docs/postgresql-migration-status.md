...
| 22 | Production Switch — Stage 7 (PostgreSQL application assembly: `server/services/postgresql/app.js`) | этот же коммит — точный hash самоссылочен и не фиксируется здесь (см. `git log -1` или последний коммит на ветке) |
...
## Последние подтверждённые результаты (после Production Switch Stage 7)

| Набор | Результат |
|---|---|
| PostgreSQL (`npm run test:postgresql`, включая новый `applicationAssemblyStage7.test.js`) | 482/482 |
| PostgreSQL `concurrency.test.js` отдельно | 14/14 |
| SQLite (`npm test`, включая существующий HTTP integration suite) | 333/333 |
| **Итого** | **815/815** (0 failed) |

Полный агрегатный прогон (`test/postgresql/*.test.js`) — два чистых прогона
подряд, без флапанья (482/482 оба раза). Stage 7 suite
(`applicationAssemblyStage7.test.js`) — 39/39, запущен 4 раза подряд,
стабильно зелёный на всех прогонах (включая тайминг-чувствительные
readiness-gate/shutdown тесты). На первом (до правок) прогоне агрегата
встретился уже известный, несвязанный с Stage 7 flake
(`orderServiceWave5.test.js` — SERIALIZABLE-retry timing под нагрузкой
параллельных test-файлов, тот же класс, что документирован в Stage 6) —
...
1. **Stage 8 — сам Production Switch** (инфраструктурный: реальный
   VPS/staging деплой, DNS/reverse proxy, создание реальной production
   PostgreSQL БД) — не выполнялся. Stage 7 подготовила полностью собранное,
   протестированное PostgreSQL-приложение (bootstrap → readiness-гейт →
   business-маршруты → graceful shutdown), готовое для staging-прогона.
...
4. **`YookassaProvider.verifyWebhook()`** — единственный метод провайдера,
   ...
   payment-webhook независимо от БД-движка (webhook-маршрут это переживает
   как обычную 500-ошибку, см. Stage 7).
