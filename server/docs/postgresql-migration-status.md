...
| 21 | Production Switch — Stage 6 (PostgreSQL operational infrastructure: bootstrap/health/lifecycle) | этот же коммит — точный hash самоссылочен и не фиксируется здесь (см. `git log -1` или последний коммит на ветке) |
...
Ничего не осталось из этой категории после Stage 6 — унаследованные от бота
(Stage 3) два пункта (отсутствие ownership-проверки, `telegram_chat_id` не
`UNIQUE`) и маршрутный баг admin.js (Stage 4) — все являются УЖЕ
СУЩЕСТВУЮЩИМИ продуктовыми/архитектурными решениями SQLite-оригинала,
зафиксированными для будущего явного product/security-решения, не багами
переноса. Ни Stage 5, ни Stage 6 не добавили новых рисков этой категории —
все проверенные race conditions (Stage 5) оказались безопасны по конструкции,
а Stage 6 — чисто инфраструктурная задача, не затрагивающая бизнес-логику.
...
## Последние подтверждённые результаты (после Production Switch Stage 6)

| Набор | Результат |
|---|---|
| PostgreSQL (`npm run test:postgresql`, включая новый `operationalStage6.test.js`) | 443/445 passed, 2 skipped (явно, с причиной) |
| PostgreSQL `concurrency.test.js` отдельно | 14/14 |
| SQLite (`npm test`, включая существующий HTTP integration suite) | 333/333 |
| **Итого** | **790/792** (2 skipped, 0 failed) |

Полный агрегатный прогон (`test/postgresql/*.test.js`) — два чистых прогона
подряд, без флапанья (443/445 + 2 skip оба раза). Stage 6 suite
(`operationalStage6.test.js`) — 28/28, запущен 5 раз подряд, стабильно
зелёный на всех прогонах (включая тайминг/сигнал-чувствительные lifecycle-
тесты). На прогонах агрегата этой задачи по разу встретились два РАЗНЫХ,
уже известных несвязанных с Stage 6 flake (`orderServiceWave5.test.js` —
SERIALIZABLE-retry timing; `orderServiceWave2.test.js` — lock-wait timing под
нагрузкой параллельных test-файлов) — оба подтверждены стабильно зелёными
при изолированном перезапуске; ни один не касается Stage 6 кода.
...
1. **Stage 7 — staging-прогон** и **Stage 8 — сам Production Switch**
   (инфраструктурный, DNS/reverse proxy) — не выполнялись. Stage 6 подготовил
   операционный скелет (bootstrap/health/lifecycle), но НЕ смонтировал
   реальные бизнес-маршруты (`routes/postgresql/api.js`/`admin.js`/bot) в
   `server.postgresql.js` — это сознательно оставлено Stage 7/8 (см.
   `postgresql-operational-readiness.md`).
...
5. **Продуктовое решение по найденному маршрутному багу admin.js**
   (`/restaurants/:id/` перехватывает `/restaurants/:id`) — затрагивает
   ЖИВОЙ SQLite-admin, не блокер Stage 7+, но требует отдельного явного
   решения перед реальным использованием формы редактирования ресторана
   (см. `postgresql-admin-port.md`).
6. **Реальный сетевой оркестратор `ensureRefundReady()` (refund) и
...
7. **Reconciliation improvements** (опционально) — подключение дормантного
   `getRefund()`, опциональный refund-webhook.
8. **Продуктовое решение по двум унаследованным от SQLite-оригинала
   ограничениям bot'а** (не блокер Stage 7+, но требует отдельного явного
