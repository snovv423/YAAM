...
## YooKassa webhook — известный блокер (не решён в Stage 7)

`YookassaProvider`'s `verifyWebhook()` — ЕДИНСТВЕННЫЙ метод провайдера,
до сих пор бросающий `throw new Error('not implemented')`; конструктор
провайдера ТАКЖЕ требует `YOOKASSA_SHOP_ID`/`YOOKASSA_SECRET_KEY`. Это
долгоживущий, задокументированный ещё с более ранних этапов блокер —
Stage 7 НЕ переписывает платёжную архитектуру и НЕ реализует его
("не придумывай ложную безопасность"). Маршрут переживает этот сбой как
обычную ошибку — 500, generic-сообщение, без утечки деталей (проверено
тестом `F2`), не крашится, не течёт stack trace. Реальный production-
webhook YooKassa заблокирован этим НЕЗАВИСИМО от выбора БД-движка.

Идемпотентность повторной доставки webhook (duplicate delivery) проверена
на уровне САМОГО МАРШРУТА (`markPaid`/`markPaymentFailed` — уже доказанно
идемпотентны с Wave 1-2) через тестовый monkey-patch `paymentService.
verifyWebhook`, возвращающий фиксированный валидный event — НЕ через
настоящую верификацию подписи ЮKassa (которая недоступна из-за блокера
выше).
...
(pause-expiry scheduler И bot-адаптер, если бот включён) → закрыть
