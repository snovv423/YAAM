...
test('A5: успешный логин в clean-root — редирект на "/", НЕ "/hq"; «Обзор» не содержит подстроки "/hq" нигде в HTML', withHqApp({}, async ({ port }) => {
  ...
  // НЕ голая проверка includes('/hq') — Stage 4 добавил на каждую HQ-страницу
  // <script src=".../static/hq.js">; сам файл ЗАКОНОМЕРНО называется "hq.js"
  // (Stage 2), и "/static/hq.js" случайно, но безобидно содержит подстроку
  // "/hq" (символ "/" перед именем файла + первые две буквы имени) — это НЕ
  // утечка префикса `/hq` как сегмента пути, только текстовое совпадение с
  // именем файла. Проверяем именно "/hq" как ГРАНИЦУ сегмента пути (за ним
  // "/", кавычка, "?" или конец строки) — реальный leaked-префикс "/hq/..."
  // или ="/hq" сюда по-прежнему точно попадёт, а "/hq.js" — корректно нет.
  assert.ok(!/\/hq(?:[/"?]|$)/.test(overviewHtml), '«Обзор» в clean-root режиме не должен содержать "/hq" как отдельный сегмент пути');
}));
