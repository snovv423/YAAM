...
test('L7: повторный запуск ничего не применяет', SKIP, async () => {
  ...
  // "повторный запуск ничего не применяет" ниже это не меняет.
  ...
  assert.deepEqual(rows.map((r) => r.version), [1, 2, 3, 4, 5, 6, 7, 8, 9]);
  await db.close();
});
...
test('L13: конкурентный запуск применяет каждую миграцию один раз', SKIP, async () => {
  ...
  assert.equal(total, 9, 'суммарно применено ровно девять миграций (UX fix-stage после Stage 28/29 добавила 0007, 0008 и 0009)');
  await db.close();
});
...
test('L14: пустая база проходит строго 0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006 -> 0007 -> 0008 -> 0009', async () => {
  ...
  assert.deepEqual(result.applied.map((a) => a.version), [1, 2, 3, 4, 5, 6, 7, 8, 9]);
...
test('L16: 0003 не содержит разрушающих операций и не трогает hq_sessions', () => {
  ...
  assert.deepEqual(files.map((f) => f.version), [1, 2, 3, 4, 5, 6, 7, 8, 9]);
