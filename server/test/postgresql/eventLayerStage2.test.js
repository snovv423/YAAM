...
test('cancelByCustomer: два конкурентных вызова — ровно одна эмиссия, один rejects', async () => {
  const restaurantId = await pgCreateRestaurant();
  const order = await pgCreateOrder(restaurantId, { status: 'awaiting_restaurant' });
  await pgCreatePayment(order.id, { status: 'succeeded' });
  ...
  try {
    ...
    assert.equal(cap.captured.length, 1, 'проигравший throw-гвардом не эмитит вообще (исключение до emit-строки)');
  } finally {
    cap.stop();
  }
});
