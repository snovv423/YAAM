...
// Stage 4 — та нашла и ЗАДОКУМЕНТИРОВАЛА этот маршрутный баг, буквально
// скопировав порядок регистрации SQLite-оригинала ("сохранить поведение
// максимально точно" — переставлять порядок было вне мандата Stage 4).
//
// Production Switch — Stage 7: этому найденному багу было явно разрешено
// получить ЛОКАЛЬНЫЙ фикс (порядок двух router.post() в routes/postgresql/
// admin.js поменян местами — см. комментарий там же) — задание Stage 7
// прямо требовало не чинить его молча: воспроизвести тестом (это и был C3
// исходно), затем чинить, если фикс локальный/безопасный/не меняет внешний
// контракт. Тест ниже обновлён под НОВОЕ, исправленное поведение — реальный
// UPDATE теперь побеждает. SQLite-оригинал (routes/admin.js) НЕ тронут, там
// баг остаётся ровно таким, каким его нашла Stage 4.
test('C3 (исправлено в Stage 7): редактирование ресторана через реальный UPDATE-путь теперь достижимо — реальный UPDATE побеждает "/:id/"-заглушку', async () => {
  ...
  assert.equal(rows[0].name, 'Обновлённое имя', 'после Stage 7 фикса реальный UPDATE-обработчик достижим — данные должны были обновиться');
});
...
test('C9 (исправлено в Stage 7): POST на несуществующий ресторан — реальный UPDATE, безопасный no-op (WHERE не матчит строк), редирект на /edit, который сам отдаёт 404', async () => {
  // После фикса C3 запрос доходит до реального UPDATE-обработчика — тот не
  // проверяет rowCount (см. комментарий в routes/postgresql/admin.js): при
  // несуществующем id WHERE просто не матчит ни одной строки (безопасный
  // no-op), обработчик всё равно редиректит на /edit, а уже ТА страница
  // сама проверяет существование и корректно отдаёт 404.
  const res = await postForm('/admin/restaurants/999999999', { name: 'X', cities: 'Y' });
  assert.equal(res.status, 302);
  assert.equal(res.headers.get('location'), '/admin/restaurants/999999999/edit');
