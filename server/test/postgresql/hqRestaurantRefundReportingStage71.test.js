...
test('I: /hq/finance и вкладка «Статистика» показывают «Возвращено клиентам» с точной суммой/количеством', async () => {
  ...
  try {
    ...
    assert.match(financeHtml, /Возвращено клиентам/, 'экран «Финансы» должен использовать новую подпись');
    assert.doesNotMatch(financeHtml, /Успешные возвраты/, 'старая подпись не должна остаться нигде');
    assert.match(financeHtml, /1\s*шт\s*·\s*333\s*₽/, 'точная сумма/количество должны быть видны');
    assert.match(financeHtml, /Возвраты показаны отдельно и не вычитаются повторно/, 'пояснение о том, что возврат не вычитается повторно, должно быть на экране');
    ...
    assert.match(statsHtml, /Возвращено клиентам/, 'вкладка «Статистика» ресторана должна использовать ту же новую подпись');
    assert.match(statsHtml, /1\s*шт\s*·\s*333\s*₽/);
  } finally {
