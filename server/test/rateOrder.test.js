...
test('средний рейтинг пересчитывается корректно', async () => {
  ...
  const { order } = await orderService.createOrder(basicOrderPayload(rid, mid, { customerPhone: '+79286660004' }))
    .then(async (res) => {
      const paymentRow = db.prepare("SELECT id FROM payments WHERE order_id = ? AND status = 'pending'").get(res.order.id);
      await orderService.markPaid(res.order.id, paymentRow.id);
